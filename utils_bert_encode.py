...
def sample_image(model, encoder, output_image_dir, n_row, batches_done, dataloader, device, bert_decoder):
    ...
    while not done:
        for (_, labels_batch, captions_batch) in dataloader:
            ...
            conditional_embeddings = conditional_embeddings.unsqueeze(dim=2).unsqueeze(dim=3)
            conditional_embeddings = bert_decoder.forward(conditional_embeddings)
