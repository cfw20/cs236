import argparse
import os
from data_bert_proj_simdeepsnx_cb_im import CIFAR10Dataset, Imagenet32Dataset, Imagenet32Subset
from models.embedders import BERTEncoder, OneHotClassEmbedding, UnconditionalClassEmbedding
import torch
from models.cgan_proj_simdeepsnx_cb_sa import CDCGAN_G, CDCGAN_D
#from models.sndcgan import CDCGAN_G, CDCGAN_D
from utils.utils import sample_image, load_model, evaluate
...
parser.add_argument("--output_dir", type=str, default="outputs/cgan", help="directory to store the sampled outputs")
...
parser.add_argument("--dataset", type=str, default="cifar10", choices=["imagenet32", "cifar10", "imagenetsub"])
parser.add_argument("--conditioning", type=str, default="one-hot", choices=["unconditional", "one-hot", "bert"])
parser.add_argument("--embed_dim", type=int, default=10, help="size of label embedding")


def train(model_G, model_D, embedder, optimizer_G, optimizer_D, scheduler_G, scheduler_D,
          train_loader, val_loader, adv_loss, opt, onehot_encoder):
    ...
    for epoch in range(opt.n_epochs):
        ...
        for i, (imgs, labels, captions) in enumerate(train_loader):
            ...
            if epoch < 10:
                d_noise = 0.2 * torch.FloatTensor(opt.batch_size, 1).fill_(1.0).to(device)
                d_noise[:int(opt.batch_size/10), :] = 1.0 
            elif epoch >= 10 and epoch < 20:
                d_noise = 0.1 * torch.FloatTensor(opt.batch_size, 1).fill_(1.0).to(device)
...
if __name__ == "__main__":
    ...
    if opt.dataset == "imagenetsub":
        train_dataset = Imagenet32Subset(train=not opt.train_on_val, max_size=1 if opt.debug else -1)
        val_dataset = Imagenet32Subset(train=0, max_size=1 if opt.debug else -1)
...
#        encoder = BERTEncoder()
    ...
    model_G = CDCGAN_G(z_dim=opt.z_dim, embed_dim=opt.embed_dim, n_filters=opt.n_filters)
...
#        encoder = torch.nn.DataParallel(encoder)
    ...
    onehot_encoder = onehot_encoder.to(device)
...
#    optimizer_D = torch.optim.Adam(model_D.parameters(), lr=4*opt.lr, betas=(0.5, 0.999))
    optimizer_G = torch.optim.Adam(model_G.parameters(), lr=opt.lr, betas=(0.0, 0.9))
    optimizer_D = torch.optim.Adam(model_D.parameters(), lr=4*opt.lr, betas=(0.0, 0.9))
    ...
    if opt.train:
        train(model_G=model_G, model_D=model_D, embedder=encoder, optimizer_G=optimizer_G, optimizer_D=optimizer_D,
              scheduler_G=scheduler_G, scheduler_D=scheduler_D, train_loader=train_dataloader, 
              val_loader=val_dataloader, adv_loss=adversarial_loss, opt=opt, onehot_encoder=onehot_encoder)
    else:
        ...
        print("Model loaded.")
        evaluate(model_G=model_G, embedder=encoder, test_loader=val_dataloader, device=device, p=opt.model_checkpoint)
