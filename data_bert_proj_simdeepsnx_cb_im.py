...
class Imagenet32Subset(CaptionedImageDataset):
    def __init__(self, root="datasets/ImageNet32", train=True, max_size=-1):
        '''
        :param dirname: str, root dir where the dataset is downloaded
        :param train: bool, true if train set else val
        ...
        self.classId2className = load_vocab_imagenet(os.path.join(root, "map_clsloc.txt"))
        self.classId2className2 = load_vocab_imagenet(os.path.join(root, "map_clsloc2.txt"))
        self.classId2className3 = load_vocab_imagenet(os.path.join(root, "map_clsloc3.txt"))
        data_files = sorted(os.listdir(self.dirname))
        self.images = []
        self.labelIds = []
        ...
        self.labelIds = np.concatenate(self.labelIds)
        self.labelNames = [self.classId2className[y] for y in self.labelIds]
        self.labelNames2 = [self.classId2className2[y-1] for y in self.labelIds]
        self.labelNames3 = [self.classId2className3[y-1] for y in self.labelIds]
        ...
        la = []
        na = []
#        sub_cats = ['motor vehicle', 'garment', 'sporting dog', 'monkey', 'furniture', 'lizard', 'snake', 'musical instrument']
        sub_cats = ['motor vehicle']
        for i in range(len(self.labelNames)):
#            print(self.labelNames3[i])
            if self.labelIds[i] < self.n_classes:
                im.append(self.images[i])
                la.append(self.labelIds[i])
                na.append(self.labelNames[i])
        self.images = np.asarray(im)
        self.labelIds = np.asarray(la)
        self.labelNames = na
        print('no of classes', len(Counter(self.labelIds)))
        print('no of images', len(self.images))
#        print(self.labelIds.shape)
#        self.n_classes = len(set(self.labelIds))
